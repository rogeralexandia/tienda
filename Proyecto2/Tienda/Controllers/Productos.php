    ...
    public function registrar()
{
    if (isset($_POST['nombre']) && isset($_POST['precio'])) {
        ...
        $destino = null;
        if (!empty($imagen['name'])) {
            $destino = 'public/img/productos/' . $nombreImg;
        } else if (!empty($fotoActual)) {
            $destino = $fotoActual;
        }
        ...
        if (empty($nombre) || empty($precio)) {
            ...
            if (empty($id)) {
                if (empty($imagen['name'])) {
                    ...
                    $data = $this->model->registrar($nombre, $descripcion, $precio, $cantidad, $nombreImg, $categoria);
                    if ($data > 0) {
                        move_uploaded_file($tmp_name, $destino);
                        $respuesta = array('msg' => 'Producto registrado', 'icono' => 'success');
                    } else {
                ...
                $temp = $this->model->getProducto($id);
                $data = $this->model->modificar($nombre, $descripcion, $precio, $cantidad, $nombreImg, $categoria, $id);
                if ($data == 1) {
                    if (!empty($imagen['name'])) {
                        if (file_exists('public/img/productos/' . $temp['imagen'])) {
                            unlink('public/img/productos/' . $temp['imagen']);
